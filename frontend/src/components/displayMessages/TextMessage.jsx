...
import { editMessageRoute, updateMessageReaction } from '../../Routes';
...
export default function TextMessage({ message, senderDetails, handleDelete, messageSide}) {
    ...
    const [isEditable] = useState((new Date() - new Date(message.sendAt)) < editTimeLimit);
    ...
    return (
        <div className='flex' onDoubleClick={()=>dispatch(setParentMessage(message))}>
            <div className={`relative hidden h-full group-hover/reaction:flex items-center`}>
                <div className="absolute flex -left-10 top-1 text-black text-2xl items-center">
